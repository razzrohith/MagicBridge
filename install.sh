...
#    8. Sets hostname to "raj", enables SSH password login
# =============================================================
set -euo pipefail
...
if [[ ! -f "$CERT" ]]; then
    ...
    openssl req -x509 -newkey rsa:2048 -sha256 -days 3650 -nodes \
        ...
        -addext "subjectAltName=DNS:magicbridge.local,DNS:raj.local,IP:127.0.0.1" \
        2>/dev/null
    chmod 600 "$KEY"
...
# Ensure password auth on (required for raj / lol)
...
# 12. USER (raj / lol)
# ══════════════════════════════════════════════════════════════════════════════
info "Ensuring user 'raj'…"
if ! id -u raj &>/dev/null; then
    useradd -m -s /bin/bash raj
    ok "User 'raj' created"
fi
echo "raj:lol" | chpasswd
usermod -aG sudo,video,input,dialout raj 2>/dev/null || true
ok "User 'raj' configured (password: lol)"
...
echo -e "${BOLD}${GREEN}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "  ${BOLD}KVM interface${NC}     https://raj.local/"
echo -e "  ${BOLD}Admin panel${NC}       https://raj.local/stealth/"
echo -e "  ${BOLD}SSH${NC}               ssh raj@raj.local  (password: lol)"
...
echo -e "  Open ${BOLD}https://raj.local/${NC} on any device on the same network."
echo ""
